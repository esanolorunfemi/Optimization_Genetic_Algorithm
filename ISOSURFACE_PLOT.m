...
h = colorbar; caxis([0,15]); ylabel(h,'Distance (mm)','FontSize',12,'Rotation',90); % set(h, 'ylim', [0 12.45])
daspect([1 1 1])
axis tight
camlight
lighting gouraud
...
cdata = Q;
figure(1)
set(gcf, 'PaperUnits', 'inches', 'PaperPosition', [0, 0, 3.25, 2.5])
p = patch(isosurface(i, j, k, data, 15));
isonormals(i, j, k, data, p)
isocolors(i, j, k, cdata, p)
p.FaceColor = 'interp';
...
zlabel('Z (mm)');
zticks(15);
caxis([0, 15]); % Set the color axis limits
daspect([1, 1, 1])
axis tight
camlight
lighting gouraud
